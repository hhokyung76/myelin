...
public class LtmMyelinDoingPlanScheduler {
	...
	public void doingPlanProcess()  {
		...
		List<Map<String, Object>> list = prestoTemplate.queryForList(
				"select cust_id, cust_email, myelin_flag, myelin_subject, myelin_content, first_time from myelin_contents_plan where p_minute='"
						+ currTimeFullMinute + "' ");
		for (int kk = 0; kk < list.size(); kk++) {
			...
			contentPlan.setMyelinFlag((String) tempMap.get("myelin_flag"));
			contentPlan.setMyelinSubject((String) tempMap.get("myelin_subject"));
			contentPlan.setMyelinContent((String) tempMap.get("myelin_content"));
